...
import { Plus, Trash2, MapPin, Cloud, Loader2, X, Check, Navigation } from 'lucide-react'
...
export default function RoutesPage() {
  ...
  function handleOriginChange(val: string) {
    ...
    originTimer.current = setTimeout(async () => {
      try { setOriginSuggestions((await searchKeyword(val)).slice(0, 5)) }
  ...
  function handleDestChange(val: string) {
    ...
    destTimer.current = setTimeout(async () => {
      try { setDestSuggestions((await searchKeyword(val)).slice(0, 5)) }
  ...
  const mapPolylines = activeOption
    ? [{ path: activeOption.path, color: ROUTE_COLORS[selectedOption] ?? '#3B82F6', weight: 5 }]
    : []
          ...
          {adding && (
            <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm flex flex-col gap-3">
                ...
                {originSuggestions.length > 0 && (
                  <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden">
                    {originSuggestions.map((p) => (
                      <button key={p.id} onMouseDown={() => selectOrigin(p)} className="w-full text-left px-3 py-2.5 hover:bg-blue-50 transition-colors border-b border-gray-50 last:border-0">
                ...
                {destSuggestions.length > 0 && (
                  <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden">
                    {destSuggestions.map((p) => (
                      <button key={p.id} onMouseDown={() => selectDest(p)} className="w-full text-left px-3 py-2.5 hover:bg-blue-50 transition-colors border-b border-gray-50 last:border-0">
                        <p className="text-sm text-gray-800">{p.place_name}</p>
                        <p className="text-xs text-gray-400 mt-0.5 truncate">{p.road_address_name || p.address_name}</p>
                      </button>
                    ))}
              ...
              <div className="flex gap-2">
                <button onClick={handleAdd} disabled={saving} className="flex-1 flex items-center justify-center gap-1 bg-blue-500 hover:bg-blue-600 text-white text-sm py-2 rounded-lg transition-colors disabled:opacity-60">
                  {saving ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} />} 저장
                </button>
                <button onClick={resetForm} className="flex-1 flex items-center justify-center gap-1 bg-gray-100 hover:bg-gray-200 text-gray-600 text-sm py-2 rounded-lg transition-colors">
                  <X size={13} /> 취소
                </button>
              </div>
            </div>
                    ...
                    <div className="border-t border-gray-100 px-4 py-3">
                      {loadingDirections ? (
                        ...
                        <div className="flex flex-col gap-2">
                          <p className="text-xs text-gray-400 flex items-center gap-1"><Navigation size={11} /> 경로 선택</p>
